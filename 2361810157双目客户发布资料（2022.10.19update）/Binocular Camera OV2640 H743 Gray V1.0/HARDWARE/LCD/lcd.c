...
extern SRAM_HandleTypeDef hsram4;    //SRAM句柄(用于控制LCD)
//LCD的画笔颜色和背景色	   
...
//    else
    {    
        ...
        GPIO_Initure.Pin=GPIO_PIN_15;           //PB5,背光控制
...
//        hsram4.Instance=FMC_NORSRAM_DEVICE; //BANK1
//        hsram4.Extended=FMC_NORSRAM_EXTENDED_DEVICE;       
//        
//        hsram4.Init.NSBank=FMC_NORSRAM_BANK4;     //使用NE1
//        hsram4.Init.DataAddressMux=FMC_DATA_ADDRESS_MUX_DISABLE;  //不复用数据线
//        hsram4.Init.MemoryType=FMC_MEMORY_TYPE_SRAM;              //SRAM
//        hsram4.Init.MemoryDataWidth=FMC_NORSRAM_MEM_BUS_WIDTH_16; //16位数据宽度
//        hsram4.Init.BurstAccessMode=FMC_BURST_ACCESS_MODE_DISABLE; //是否使能突发访问,仅对同步突发存储器有效,此处未用到
//        hsram4.Init.WaitSignalPolarity=FMC_WAIT_SIGNAL_POLARITY_LOW;//等待信号的极性,仅在突发模式访问下有用
//        hsram4.Init.WaitSignalActive=FMC_WAIT_TIMING_BEFORE_WS;   //存储器是在等待周期之前的一个时钟周期还是等待周期期间使能NWAIT
//        hsram4.Init.WriteOperation=FMC_WRITE_OPERATION_ENABLE;    //存储器写使能
//        hsram4.Init.WaitSignal=FMC_WAIT_SIGNAL_DISABLE;           //等待使能位,此处未用到
//        hsram4.Init.ExtendedMode=FMC_EXTENDED_MODE_ENABLE;        //读写使用不同的时序
//        hsram4.Init.AsynchronousWait=FMC_ASYNCHRONOUS_WAIT_DISABLE;//是否使能同步传输模式下的等待信号,此处未用到
//        hsram4.Init.WriteBurst=FMC_WRITE_BURST_DISABLE;           //禁止突发写
//        hsram4.Init.ContinuousClock=FMC_CONTINUOUS_CLOCK_SYNC_ASYNC;
...
//		
//        HAL_SRAM_Init(&hsram4,&FSMC_ReadWriteTim,&FSMC_WriteTim);		
//        HAL_Delay(50); // delay 50 ms 
	...
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//如果是这几个IC,则设置WR时序为最快
	{
        ...
        FMC_NORSRAM_Extended_Timing_Init(hsram4.Extended,&FSMC_WriteTim,hsram4.Init.NSBank,hsram4.Init.ExtendedMode);
	} 
    //printf("LCD ID:%x\r\n",lcddev.id); //打印LCD ID  	
